...
double ffopt(int ndofAll, double *dofAll, double *dofgradAll, void *data)
{
  ...
  PetscScalar *dof_filtered,*dofgrad_filtered;
  dof_filtered = (PetscScalar *) malloc(ndof*sizeof(PetscScalar));
  dofgrad_filtered = (PetscScalar *) malloc(ndof*sizeof(PetscScalar));

  filters_apply(subcomm,dof,dof_filtered,flt,1);
  PetscScalar xfar=ffgrad(subcomm, dof_filtered,dofgrad_filtered, M0,b,x, epsDiff,epsBkg, omega, ffvec, ksp,its,maxit, dofi,dg.da);
  filters_apply(subcomm,dofgrad_filtered,dofgrad,flt,-1);
