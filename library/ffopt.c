...
#undef __FUNCT__
#define __FUNCT__ "ffopt_maximinconstraint"
double ffopt_maximinconstraint(int ndofAll_with_dummy, double *dofAll_with_dummy, double *dofgradAll_with_dummy, void *data)
{
  ...
  double obj=ffopt(ndofAll,dofAll,dofgradAll,data);
...
double dummy_obj(int ndofAll_with_dummy, double *dofAll_with_dummy, double *dofgradAll_with_dummy, void *data)
{
  ...
  char tmpstr[PETSC_MAX_PATH_LEN];
  sprintf(tmpstr,"prepping dummy objective at step %d (global timecheck)",count);
  updateTimeStamp(PETSC_COMM_WORLD,&global_time,tmpstr);
  ...
  sprintf(tmpstr,"prepping-done dummy objective at step %d (global timecheck)",count);
  updateTimeStamp(PETSC_COMM_WORLD,&global_time,tmpstr);
