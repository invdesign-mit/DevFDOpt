...
PetscErrorCode main(int argc, char **argv)
{
  ...
  for(i=0;i<nfreq;i++){
    for(j=0;j<nangle;j++){
      ...
      ff2dxz_asymp(subcomm,dg.da,ffvec[jpt],r_far,theta_far[jpt]*(M_PI/180),freqs[i],x_offset,&zp_ref,hx,hz,vec_verbose);
    }
  }
  MPI_Barrier(subcomm);
  MPI_Barrier(PETSC_COMM_WORLD);
  ...
  if(Job==0){
    ...
    if(opt_num==1){
      ...
      double *lb,*ub;
      lb=(double *) malloc(dofi.ndof*sizeof(double));
      ub=(double *) malloc(dofi.ndof*sizeof(double));
      make_array(lb,0.0,dofi.ndof);
      make_array(ub,1.0,dofi.ndof);
      optimize_generic(dofi.ndof, dofcell, lb,ub, ffdat+jpt, NULL, ffopt, NULL, 1, 0);
