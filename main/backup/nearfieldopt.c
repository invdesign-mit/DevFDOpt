...
PetscErrorCode main(int argc, char **argv)
{
  ...
  dispersiveV_2d(subcomm,dg.da, v0,v1,v2,v3,v4,u,    planewave_phaseonly,&pw_out, x_offset,&zref,hx,hz,1);
  ...
  if(Job==0){
    ...
    sprintf(tmpstr,"epsilon_cell%03d.h5",colour);
    ...
    if(print_Efields){
      ...
      SolveMatrixDirect(subcomm,ksp,M0,b0,x0,&its,maxit);
      sprintf(tmpstr,"Efield_cell%03d.h5",colour);
      saveVecHDF5(subcomm,x0,tmpstr,"E");
    }
    ...
    PetscReal amp_lb,amp_ub;
    getreal("-amp_lb",&amp_lb,0);
    getreal("-amp_ub",&amp_ub,1.0/0.0);
    double lb[2]={amp_lb, 0}, ub[2]={amp_ub, 2*M_PI};
    optimize_generic(2, amp_dof, lb,ub, &data, NULL, phopt_dispsum_amponly, NULL, 1, 0);
    MPI_Barrier(subcomm);
    MPI_Barrier(PETSC_COMM_WORLD);
    ...
    optimize_generic(ndofAll, dofAll, lb,ub, &data, NULL, phopt_dispsum_epsglobal_globalabs, NULL, 0, 0);
    MPI_Barrier(subcomm);
    MPI_Barrier(PETSC_COMM_WORLD);
