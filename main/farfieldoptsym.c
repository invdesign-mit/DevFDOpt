...
PetscErrorCode main(int argc, char **argv)
{
  ...
  PetscReal amp_real[nfreq*nangle];
  getrealarray("-amp_real",amp_real,&nget,1.0);
  ...
  for(i=0;i<nfreq;i++){
    for(j=0;j<nangle;j++){
      ...
      PetscScalar amp=amp_real[jpt]+PETSC_i*0;
  ...
  if(Job==0){
    ...
    double *dofsym=(double *) malloc(dofi.ndof*ncells*sizeof(double));
    int sym;
    getint("-sym",&sym,0);
    if(sym){
      symx(&(dofAll[0]), dofsym, dofi.Mx,dofi.My,dofi.nlayers,ncells/2,1, 0);
      //symx(&(dofAll[dofi.ndof*ncells/2]), dofsym, dofi.Mx,dofi.My,dofi.nlayers,ncells/2,1, 0);
    ...
    if(opt_num==1){
      ...
      int ndofhalf=dofi.ndof*ncells/2;
      double *lb,*ub;
      lb=(double *) malloc(ndofhalf*sizeof(double));
      ub=(double *) malloc(ndofhalf*sizeof(double));
      make_array(lb,0.0,ndofhalf);
      make_array(ub,1.0,ndofhalf);
      //optimize_generic(ndofhalf, &(dofAll[ndofhalf]), lb,ub, ffdat+jpt, NULL, ffoptsym, NULL, 1, 0);
      optimize_generic(ndofhalf, &(dofAll[0]), lb,ub, ffdat+jpt, NULL, ffoptsym, NULL, 1, 0);
	...
	id=j*opt_nfreq+i;
	data[id]=ffdat+jpt;
	funcs[id]=ffoptsym_maximinconstraint;
	} 
      }
    ...
    //int ntot=dofi.ndof*ncells/2+1;
    int ntot=dofi.ndof*ncells/2;
    double *doftot,*dofgradtot;
    doftot = (double *) malloc(ntot*sizeof(double));
    dofgradtot = (double *) malloc(ntot*sizeof(double));
    for(i=0;i<ntot-1;i++){
      //doftot[i]=dofAll[i+dofi.ndof*ncells/2];
      doftot[i]=dofAll[i];
    }
    doftot[ntot-1]=0.01;
    ...
    for(s=s0;s<s1;s+=ds){
      doftot[p]=s;
      objval=ffoptsym(ntot,doftot,dofgradtot,ffdat+jpt);
