...
#undef __FUNCT__
#define __FUNCT__ "createP"
/**
 * createP
 * --------
 * Create the matrix P, the permutation operator on E fields for transforming to and from disjoint cells.
 * E_seamless = P E_segmented
 * E_segmented = P^T E_seamless
 * M_segmented = P^T M_seamless P
 */
PetscErrorCode createP(MPI_Comm comm, Mat *P, ParDataGrid dg_entire, PetscInt Nx, PetscInt Ny, PetscInt Nz, PetscInt nx, PetscInt ny, PetscInt nz)
{
	PetscFunctionBegin;
	PetscErrorCode ierr;
	
	ierr = MatCreate(comm, P); CHKERRQ(ierr);
	ierr = MatSetSizes(*P, dg_entire.Nlocal_tot, dg_entire.Nlocal_tot, PETSC_DETERMINE, PETSC_DETERMINE); CHKERRQ(ierr);
	ierr = MatSetType(*P, MATRIX_TYPE); CHKERRQ(ierr);
	ierr = MatSetFromOptions(*P);
	ierr = MatMPIAIJSetPreallocation(*P, 1, PETSC_NULL, 1, PETSC_NULL); CHKERRQ(ierr);
	ierr = MatSeqAIJSetPreallocation(*P, 1, PETSC_NULL); CHKERRQ(ierr);
	ierr = MatSetLocalToGlobalMapping(*P, dg_entire.map, dg_entire.map); CHKERRQ(ierr);
	ierr = MatSetStencil(*P, Naxis, dg_entire.Nlocal_g, dg_entire.start_g, Naxis); CHKERRQ(ierr);

	PetscInt mx=Nx/nx, my=Ny/ny, mz=Nz/nz;
	
	/** Get corners and widths of Yee's grid included in this proces. */
	PetscInt _ox, _oy, _oz;  // coordinates of beginning corner of Yee's grid in this process
	PetscInt _lx, _ly, _lz;  // local widths of Yee's grid in this process
	ierr = DMDAGetCorners(dg_entire.da, &_ox, &_oy, &_oz, &_lx, &_ly, &_lz); CHKERRQ(ierr);

	//ic_t,ix_t,iy_t,iz_t -> (tilde) seamless order
	//ic,ix,iy,iz,jx,jy,jz -> segmented order
	//ic_p,ix_p,iy_p,iz_p -> Petsc global order from segments
	PetscInt ic_t, ix_t, iy_t, iz_t;
	PetscInt ic, ix, iy, iz, jx, jy, jz;
	PetscInt ic_p, ix_p, iy_p, iz_p;
	PetscInt i,j,k,p;
	PetscInt Nc=Naxis, nc=Naxis;
	MatStencil ind_t, ind_p;
	for (iz_t = _oz; iz_t < _oz+_lz; ++iz_t) {
	  for (iy_t = _oy; iy_t < _oy+_ly; ++iy_t) {
	    for (ix_t = _ox; ix_t < _ox+_lx; ++ix_t) {
	      for (ic_t = 0; ic_t < Nc; ++ic_t) { 

		ix = ix_t%nx, jx = ix_t/nx;
		iy = iy_t%ny, jy = iy_t/ny;
		iz = iz_t%nz, jz = iz_t/nz;
		ic = ic_t;

		i = ic + nc*ix + nc*nx*iy + nc*nx*ny*iz;
		j = jx + mx*jy + mx*my*jz;
		k = i + nc*nx*ny*nz*j;
		...
		ierr = MatSetValuesStencil(*P, 1, &ind_t, 1, &ind_p, 1.0, INSERT_VALUES); CHKERRQ(ierr);
