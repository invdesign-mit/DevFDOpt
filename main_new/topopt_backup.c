...
\tnp_per_comm = nproc_total/ncomms = 2000/50 = 40.\n");
  ...
  PetscPrintf(PETSC_COMM_WORLD,"voptions 0 [NF plane wave], 1 [FF focus 2d], 2 [FF focus 3d], 3 [NF plane wave phase error]\n");
  ...
  for(i=0;i<nfreq;i++){
	...
	its[jpt][i_cell]=1000;
      ...
      if(voptions>3) (vdat+jpt)->func=phopt;
  ...
  if(Job==0){
	...
	if(voptions==0){
	  ...
	  nget=opt_num;
	  getrealarray("-ub_ampphi",ubphi,&nget,2*M_PI);
	}else if(voptions==3){
	  lbtot[ndof_eps]=ampr;
	  ubtot[ndof_eps]=ampr;
	  for(i=ndof_eps+1;i<ndofAll;i++){
